function [my_theta_0,my_gamma,my_h,my_r] = model2(my_m_ball)
disp(['本次计算,重物质量:',num2str(my_m_ball)]);
% 环境常量
...
if flag == 0
    ...
    % s_y
    s_y_2 = @(h) (abs( F_gamma(h) .* (sec(pi./2 - phi_0(h)) - 1) / (rho_chain .* g_chain) ));
    s_y = @(h)(s_y_2(h));
...
s_x_2 =  k_incomplete .* abs(log(abs( (sec(pi./2-phi_0_h) + tan(pi./2-phi_0_h)) ))) + l_chain - k_incomplete.* tan(pi./2-phi_0_h);
